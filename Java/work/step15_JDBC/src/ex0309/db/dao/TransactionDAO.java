   ...
   * */
	public void transfer(String inputAccount, String outputAccount, int money) {
		...
		try {
			...
			//기본적으로 JDBC는 Auto-commit임
			//Auto commit 해지
			con.setAutoCommit(false);
			
			//출금하기
			int result = this.withdraw(con, outputAccount, money);
			if(result==0) throw new SQLException("출금계좌 오류로 계좌이체 실패하였습니다."); //catch로 감
			
			//입금하기
			result = this.deposit(con, inputAccount, money);
			if(result==0) throw new SQLException("입금계좌 오류로 계좌이체 실패하였습니다."); //catch로 감
			
			//잔액 확인
			if(this.balanceCheck(con, inputAccount)) {
				throw new SQLException("입금계좌의 총 금액이 1000원을 초과로 계좌이체 실패했습니다."); //catch로 감
			}
	...
	/**
	 * 잔액확인하기
	 * 입금계좌의 총 금액이 1000원을 초고하면 true, 아니면 false를 리턴한다.
	 * */

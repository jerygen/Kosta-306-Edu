...
use mytest;
 --  문자열 비교
 select strcmp("jang heejung","jang Heejung2") ; -- 오라클에서는 이런 문법 안 됨, from 테이블명 없이 하는 것
 -- 0이면 같음, -1이면 다름
 select '장희정' from dual;

SELECT ENAME ,UPPER(ENAME), LOWER(ENAME),  LENGTH(ENAME), char_length(ENAME)
  FROM EMP;
  
select '장희정' , length('장희정') , char_length('장희정') ; -- mysql에서 한글은 한글자에 3byte
...
select email, instr(email, '@') from instructor;
select email, locate('@', email) from instructor;
select email, position('@' in email) from instructor;

select * , substring(email, 1, position('@' in email) -1) as 이메일자리, substring(email, position('@' in email) + 1 ) as 이메일 from instructor;
...
-- 26-02-02 ~ 26-07-10
-- 우리가 만난지 얼마나?
select timestampdiff(day, '2026-2-2', now());
select datediff(now(), '2026-02-02') as 만난일수;

-- 우리 과정 총 기간 일수
select timestampdiff(day, '2026-2-2','2026-07-10');
select concat(timestampdiff(day, '2026-2-2','2026-07-10'), '일') as 총기간;

-- 100일은 언제?
select date_add('2026-2-2', interval 100 day);
select date_add('2026-2-2', interval 100 day), 
	dayname(date_add('2026-2-2', interval 100 day));
...
/* NULL에 관련된 함수 : IFNULL()  , COALESCE()
     - IFNULL(열, 대체할 값) :  expr1이 NULL이면 expr2를 반환, 아니면 대체할 값을 반환.
     - COALESCE(열 1, 열 2, …) : COALESCE는 NULL이 아닌 값이 나올 때까지 후보군의 여러 열을 입력할 수 있음
	 - NULLIF(열1 , 열2) : expr1과 expr2가 같으면 NULL 반환, 다르면 expr1 반환. 
 */
...
*/
-- IF 이용
select sal, if(sal>=3000, '고액년봉자', '') as 등급 from emp;


/*
EX) EMP테이블에서 DEPtNO가 10 이면 관리부, 20이면 총무부, 30이면 영업부 
...
select *, (ifnull(kor,0) + ifnull(eng, 0) + ifnull(math,0)) 합계, round((ifnull(kor,0) + ifnull(eng, 0) + ifnull(math,0))/3, 2) 평균 from report;


--  국어총점, 국어평균을 검색해보자.
select sum(kor) 합계, avg(kor) 평균, max(kor), min(kor), count(kor), count(*) from report;
-- count(kor) -> 8, count(*) -> 9 , kor에는 null이 존재하기 때문
select sum(kor) 합계, avg(kor), avg(ifnull(kor,0)), max(kor), min(kor), count(kor), count(*) from report;

-- 일반컬럼과 집계함수를 SELECT절에 함께 쓸수 있을까?? -> 안된다!!!
select name, sum(kor) 합계, avg(kor) 평균 from report; -- error 일반 컬럼과 집계함수는 같이 쓸 수 없다. 
...
select ban, max(kor), min(kor), sum(kor), avg(kor), count(*) -- 4)
from report		-- 1)
where kor>=70	-- 2)
group by ban;	-- 3)
 
-- KOR의 평균이 80 이상인 학생들의 반별 국어 최대, 최소 총점 평균 인원수 
select ban, max(kor), min(kor), sum(kor), avg(kor)-- 4)
from report		-- 1)
where avg(kor) >= 80	-- 2) 에러 발생 집계함수를 where 절에서 조건으로 사용할 수 없다.
group by ban;	-- 3)

...
select distinct ename, job from emp; -- 레코드 모두가 같아야 제거됨
select distinct job from emp; -- 단일행에 대해서 볼 때 사용한다.
...
select empno, ename, sal, comm, (sal+comm)*12 as 년봉 from emp;

-- NULL값을 다른 값으로 변경해서 연산 할 수 있다  -->  IFNULL(칼럼명, 변경값)
select comm, ifnull(comm, 100) from emp;

-- ex) 급여와 커미션을 더하고 12을 곱해서 년봉 컬럼
select empno, ename, sal, comm, (sal + ifnull(comm, 0))*12 as 년봉 from emp;

--  문자열 연결 : concat() 함수 사용가능.(~님 년봉은 ~ 입니다)
select ename, sal, concat('A','B','C') as message from emp;
select ename, sal, concat(ename, '님 년봉은 ', (sal+ifnull(comm, 0))*12 ,'입니다') as message from emp;
...
select * from emp where not sal between 2000 and 4000 order by sal;
...
insert into copy_emp(empno, ename, job, hiredate) values(9002,'king' ,'progra', now());

select * from copy_emp where ename like '%%%'; -- 모든 글자가 다 해당됨.
...
select * from copy_emp;
update copy_emp set comm = 100 where comm is null; -- 조건을 먼저 처리한 후에 업데이트 함

update copy_emp set comm = ifnull(comm, 100); -- 성능의 차이가 있음. 얘는 전체를 다 찾아서 작업을 한다
